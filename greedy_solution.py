...
def greedy_solution(vrptw: VRPTW) -> List[Route]:
    ...
    while customers_left:
        ...
        while exist_reachable_customers:
            ...
            for customer in customers_left:
                ...
                if (
                    ...
                    if customer.ready_time <= estimated_arrival:
                        ...
                        ################################################################################################################################################
                        ################################################################################################################################################
                        # if reachable_customers and get_time_between(last_delivery_x, last_delivery_y, customer.x, customer.y) < get_time_between(last_delivery_x, last_delivery_y, reachable_customers[0].x, reachable_customers[0].y):
                        #     reachable_customers.insert(0, customer)
                        # else:
                        ...
                        ################################################################################################################################################
                        ################################################################################################################################################
                        # if too_early_customers and get_time_between(last_delivery_x, last_delivery_y, customer.x, customer.y) < get_time_between(last_delivery_x, last_delivery_y, too_early_customers[0].x, too_early_customers[0].y):
                        #     too_early_customers.insert(0, customer)
                        # else:
            ...
            if reachable_customers:
                ...
                ################################################################################################################################################
                ################################################################################################################################################
                # next_customer = reachable_customers[0]
                # next_customer = reachable_customers[r.randint(0, len(reachable_customers) - 1)]
                next_customer = reachable_customers[0]
                di = get_time_between(last_delivery_x, last_delivery_y, next_customer.x, next_customer.y)
                for c in reachable_customers:
                    if (d:=get_time_between(last_delivery_x, last_delivery_y, c.x, c.y)) < di:
                        next_customer = c
                ################################################################################################################################################
                ################################################################################################################################################
                ################################################################################################################################################
