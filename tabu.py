...
from utils import fitness, fitness_vrptwless, reverse_in_same_route, exchange_route_chunk, switch_two_deliveries_with_routes, switch_two_deliveries, reverse, relocate_delivery, switch_two_deliveries_in_same_route
...
def get_neighborhood_without_tabu_list(vrptw : VRPTW, tabu):
    ...
    for operator in operators:
        neighbor, f, action = operator(vrptw, tabu)
        if f < best_f :
            best_f = f
            best_action = action
            best_neighbor = neighbor

    method_used[action[0]] += 1

    return best_neighbor, best_f, best_action

def get_neighborhood_without_tabu_list_relocate_extra(vrptw: VRPTW, tabu):
    best_f = None
    best_action = None
    best_neighbor = None
    '''
